...
@section('title', trans('admin.province.actions.edit', ['name' => $province->id]))

@section('body')
<province-form :action="'{{ $province->resource_url }}'" :data="{{ $province->toJson() }}" v-cloak inline-template>
    <div class="container-xl">
        <div class="card">
            ...
            <form class="form-horizontal form-edit" method="post" @submit.prevent="onSubmit" :action="action"
                ...
                <div class="card-header">
                    <i class="fa fa-pencil"></i> {{ trans('admin.province.actions.edit', ['name' => $province->id]) }}
                </div>

                <div class="card-body">
                    @include('admin.province.components.form-elements')
                </div>
        ...
        <district-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/provinces/districts/'.$province->id) }}'"
            inline-template>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/districts/create?provinceId='.$province->id) }}" role="button"><i
                                    class="fa fa-plus"></i>&nbsp;
                            ...
                            <div class="card-block">
                                ...
                                <table class="table table-hover table-listing">
                                    <thead>
                                        <tr>
                                            ...
                                            {{-- <th is='sortable' :column="'id'">{{ trans('admin.district.columns.id') }} --}}
                                            </th>
                                            {{-- <th is='sortable' :column="'province_id'">
                                                {{ trans('admin.district.columns.province_id') }}</th> --}}

                                            <th is='sortable' :column="'name'">
                                                {{ trans('admin.district.columns.name') }}</th>
                                ...
                                <div class="no-items-found" v-if="!collection.length > 0">
                                    ...
                                    <a class="btn btn-primary btn-spinner"
                                        href="{{ url('admin/districts/create?provinceId='.$province->id) }}"
