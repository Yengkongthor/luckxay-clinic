...
@section('title', trans('admin.exchange.actions.index'))

@section('body')

<exchange-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/exchanges') }}'" inline-template>

    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <i class="fa fa-align-justify"></i> {{ trans('admin.exchange.actions.index') }}
                    <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                        href="{{ url('admin/exchanges/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                        {{ trans('admin.exchange.actions.create') }}</a>
                </div>
                <div class="card-body" v-cloak>
                    <div class="card-block">
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    ...
                                    <th is='sortable' :column="'id'">{{ trans('admin.exchange.columns.id') }}</th>
                                    <th is='sortable' :column="'thb'">{{ __('1 THB -> KIP') }}</th>
                                    <th is='sortable' :column="'usd'">{{ __('1 USD -> KIP') }}</th>
                                    ...
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                        <span
                                            ...
                                            @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                @click="onBulkItemsClickedAll('/admin/exchanges')"
                                        ...
                                        <span class="pull-right pr-2">
                                            <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                @click="bulkDelete('/admin/exchanges/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                        </span>
                        ...
                        <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{ url('admin/exchanges/create') }}"
