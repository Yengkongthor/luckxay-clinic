...
<div class="container-xl">
    ...
    <div class="card">
        ...
        <profile-edit-profile-form :action="'{{ url('admin/profile') }}'" :data="{{ $adminUser->toJson() }}"
            inline-template v-cloak>
            <form class="form-horizontal form-edit" method="post" @submit.prevent="onSubmit" :action="action">

                @if ($adminUser->employee->position =='Doctor')
                <div class="card-header">
                    <i class="fa fa-pencil"></i>
                    {{ trans('admin.admin-user.actions.edit_profile') }}
                    <div class="float-right d-flex align-items-center ml-5">
                        <span class="mr-2">{{ __('Available') }}:</span>
                        <label class="switch switch-3d switch-success">
                            <input type="checkbox" class="switch-input" value="1" v-model="form.status"
                                @change="toggleSwitch">
                            <span class="switch-slider"></span>
                        </label>
                    </div>

                    <div class="float-right d-flex align-items-center" v-if="status == true">
                        <span class="mr-2">{{ __('Class') }}:</span>
                        <select class="form-control" id="ccmonth" v-model="form.examination_class"
                    ...
                    <div class="row">
                        <div class="col-md-4 text-center">
                            <div class="avatar-upload">
                                @include('admin.includes.avatar-uploader', [
                                'mediaCollection' =>
                                app(\Brackets\AdminAuth\Models\AdminUser::class)->getMediaCollection('avatar'),
                ...
                <div>
                    ...
                    @include('admin.partials.back-button', ['url' => route('admin/dashboard')])
                </div>
