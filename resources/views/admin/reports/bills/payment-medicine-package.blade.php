...
<table class="table1">
    ...
    <tr>
        ...
        <p>ໄດ້​ຮັບ​ເງິນ​​ຈາກທ່ານ: {{$queue->patientHistory->patient->user->name}} ອາຍຸ:
            {{$queue->patientHistory->patient->age}} ປີ,
            ບ້ານ: {{$queue->patientHistory->patient->village}} ເມືອງ:
            {{$queue->patientHistory->patient->district}} ແຂວງ:
            {{$queue->patientHistory->patient->province}} ເບີໂທ:
            {{$queue->patientHistory->patient->user->phone}}
            </p>
        </td>
    </tr>
    <tr>
...
<table class="table2">
    ...
    <tbody>
        ...
        <tr style="font-size: 10pt">
            ...
            <td style="text-align: right;font-size: 10pt;padding-right: 5px">
                {{number_format((int) $thb - (($discount_total_money/100)* (int) $thb))}} </td>
        </tr>
