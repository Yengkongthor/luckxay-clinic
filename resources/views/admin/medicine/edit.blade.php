...
@section('title', trans('admin.medicine.actions.edit', ['name' => $medicine->id]))

@section('body')

<div class="container-xl">

    <medicine-form :action="'{{ $medicine->resource_url }}'" :data="{{ $medicine->toJson() }}" v-cloak inline-template>
        <div class="card">
            ...
            <form class="form-horizontal form-edit" method="post" @submit.prevent="onSubmit" :action="action"
                ...
                <div class="card-header">
                    <i class="fa fa-pencil"></i> {{ trans('admin.medicine.actions.edit', ['name' => $medicine->id]) }}
                </div>
    ...
    <medicine-history-listing :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/medicine-histories?medicine_id='.$medicine->id) }}'" inline-template>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">
                                            {{ trans('admin.medicine-history.columns.id') }}</th>

                                        <th is='sortable' :column="'price'">
                                            {{ trans('admin.medicine-history.columns.price') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="7">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/medicine-histories')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/medicine-histories/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
